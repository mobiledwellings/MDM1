...
/**
 * The body of a partner page, split out from PartnerPage so it can be rendered
 * to static HTML at build time.
 *
 * WHY: the partner routes were served as an empty #root. The ~1,100 words on
 * each page only existed once React ran, so crawlers that don't execute
 * JavaScript — including most AI crawlers — saw nothing but a <noscript> line.
 * scripts/prerender-seo.js now renders this component and bakes the result into
 * the static HTML, the same way it already did for the Signature Solar page.
 *
 * Everything here must render on the server: no direct document/window access
 * outside of guarded hooks, and no <Link>, so no router is needed in context.
 */
...
export function PartnerMain({ partner }: { partner: Partner }) {
